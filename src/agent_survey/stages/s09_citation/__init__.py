...
def run(
    ...
    *,
    ...
    try:
        # Get paper IDs for this topic+scope+has_pdf from paper_topics
        scope_ids = set()
        for pt in db.iter_paper_topics(
            topic_name,
            "relevance = ? AND pdf_path IS NOT NULL AND pdf_path != ''",
            [scope],
        ):
            scope_ids.add(pt["paper_id"])
        if not scope_ids:
            console.print(f"[yellow]no {scope} papers with PDF left to process[/yellow]")
            return {"processed": 0}
        ...
        for pid in scope_ids:
            ...
            if not paper:
                continue
            if not force and paper.get("citation_json") and paper["citation_json"] not in ("", "{}"):
                continue
            todo_ids.add(pid)

        rows = [db.get_paper(pid) for pid in todo_ids if db.get_paper(pid)]
        if not rows:
            console.print(f"[yellow]no papers left to process[/yellow]")
            return {"processed": 0}

        console.print(f"[bold]Building citation graph for {len(rows)} {scope} papers [{topic_name}]...[/bold]")

        # Build signatures from ALL papers in this topic+scope
        all_core = []
        for pt in db.iter_paper_topics(topic_name, "relevance = ?", [scope]):
        ...
        with Progress(console=console) as prog:
            ...
            for idx, r in enumerate(rows, 1):
                ...
                db.mark_stage(pid, "citation", "done", topic_name=topic_name)
                processed += 1
                prog.advance(task)

        # Build graph from all processed papers in this topic+scope
        all_processed = []
        for pt in db.iter_paper_topics(topic_name, "relevance = ?", [scope]):
            p = db.get_paper(pt["paper_id"])
            if p and p.get("citation_json") and p["citation_json"] not in ("", "{}"):
