"""Stage 1b: fill abstracts for failed papers using Playwright + arXiv + venue DOI.
...
ACM DL and IEEE Xplore are fetched via Playwright when the paper URL
matches those domains.  Cloudflare bot detection means headless Chromium
often fails; headless=False with --no-startup-window is used to reduce
detection, but success is venue-dependent.
"""
...
from .strategies.acm import fetch_acm_abstract
from .strategies.ieee import fetch_ieee_abstract
...
def run_web(
    ...
    *,
    ...
    try:
        ...
        # Launch shared browser once with stealth-friendly args
        ...
        def _try_web_enrich(row: dict) -> str | None:
            ...
            # Try venue-specific DOI fetchers first for ACM / IEEE
            if "dl.acm.org" in url:
                _set_worker_job(row, "acm")
                abstract = fetch_acm_abstract(browser, url)
                _clear_worker_job()
                if abstract:
                    return abstract
            elif "ieeexplore.ieee.org" in url:
                _set_worker_job(row, "ieee")
                abstract = fetch_ieee_abstract(browser, url)
                _clear_worker_job()
                if abstract:
                    return abstract
            ...
            abstract = _search_arxiv(browser, title)
            _clear_worker_job()
