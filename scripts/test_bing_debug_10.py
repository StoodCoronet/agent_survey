#!/usr/bin/env python3
"""Debug Bing search — title matching on first 10 missing surveys."""
...
def bing_search_debug(client: httpx.Client, title: str, idx: int) -> dict:
    q = quote_plus(title)
    ...
    html = r.text
    (OUT_DIR / f"{idx:02d}_bing.html").write_text(html, encoding="utf-8")
    ...
    # Strategy 2: any <a> with href and text length 15-200 (fallback)
    if not candidates:
        for m in re.finditer(r'<a[^>]+href=["\']([^"\']+)["\'][^>]*>([^<]{15,200})</a>', html, re.IGNORECASE):
            raw_url = m.group(1)
    ...
    for c in matched:
        u = c["url"]
        # Skip Bing redirects for ID extraction (often contain real URL inside)
...
def main():
    ...
    client = httpx.Client(
        ...
        proxy=os.getenv("HTTP_PROXY", "http://192.168.1.106:7890"),
    )
    summary = []
    ...
    for i, row in enumerate(rows, 1):
        ...
        res = bing_search_debug(client, title, i)
        console.print(f"[dim]HTML size: {res['html_len']}, b_algo blocks: {res['algo_blocks']}[/dim]")

        if res["candidates"]:
            console.print("[dim]Top candidates:[/dim]")
            for c in res["candidates"][:5]:
                mark = "[green]✓[/green]" if c["match"] else "[red]✗[/red]"
                src = c["source"][:3]
                console.print(f"  {mark} [{src}] {c['match']:<10} | {c['raw_title'][:60]}...")
